...
async function downloadWebHtml() {
  ...
  const styleMap = {
    ...
    '.source-links': 'background: #f1f5f9; padding: 12px; border-radius: 10px; margin-top: 10px; font-size: 13px;',
...
/* =========================================================================
   EMAIL PREVIEW & EXPORT LOGIC (WEEKLY)
   ========================================================================= */
const mjmlTemplateWeekly = `
<mjml>
  <mj-head>
  ...
  <mj-body background-color="#FAFAFA" width="700px">
    ...
    {{/if}}
    {{#if Body_Flow}}
    <mj-section padding="0 20px 12px 20px"><mj-column><mj-text font-size="18px" font-weight="800" color="#44A4FF" padding-bottom="12px"><span style="color:#39E6FD; margin-right:6px;">●</span> 주간 흐름 요약</mj-text></mj-column></mj-section>
    <mj-section padding="0 20px 32px 20px"><mj-column background-color="#FFFFFF" border="1px solid #E5E7EB" border-radius="12px"><mj-text font-size="16px" color="#1F2937" padding="18px">{{{Body_Flow}}}</mj-text></mj-column></mj-section>
    {{/if}}
    {{#if Img_Sec3}}<mj-section padding="0 20px 32px 20px"><mj-column><mj-image src="{{Img_Sec3}}" border-radius="12px" /></mj-column></mj-section>{{/if}}
    {{#if Source_List}}
...
async function openEmailPreview() {
  ...
  try {
    const template = Handlebars.compile(mjmlTemplateWeekly);
...
function downloadEmailHtml() {
  ...
  a.download = `Insight_Weekly_${dateStr}.html`;
  a.click();
}
